...
import { Save, AlertCircle, CheckCircle, Download, History, Calendar, Check, Lock, Eye, RotateCcw, MessageSquare, X, ShieldAlert, BadgeCheck } from 'lucide-react';
...
const AvailabilityScheduler: React.FC<AvailabilitySchedulerProps> = ({
    ...
    // Calculate if Submission is Open
    const today = new Date().toISOString().split('T')[0];
    const isSubmissionDateValid = (!period.submissionStart || today >= period.submissionStart) &&
        (!period.submissionEnd || today <= period.submissionEnd);
    const isOpenForSubmission = period.isOpen && isSubmissionDateValid;
    ...
    const toggleSlot = (day: string, time: string) => {
        ...
        if (readOnly || !isOpenForSubmission || viewingVersionId) return;
    ...
    const handleMarkFinal = (vId: string) => {
        if (readOnly || !isOpenForSubmission) return;
        onMarkFinal(vId);
    ...
    const isEditable = !readOnly && isOpenForSubmission && !viewingVersionId;

    // Helper to format date preventing timezone shift (treat YYYY-MM-DD as local)
    const formatDateUTC = (dateString: string) => {
        if (!dateString) return '';
        const parts = dateString.split('-');
        // Create date as noon to avoid midnight timezone shifts
    ...
    return (
        <div className="space-y-6">
            ...
            {!isOpenForSubmission && !readOnly && (
                <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg shadow-sm flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
                    <ShieldAlert className="text-red-600 shrink-0 mt-0.5" size={24} />
                    <div>
                        <h3 className="text-red-800 font-bold text-lg">Periodo de carga de disponibilidad cerrado</h3>
                        <p className="text-red-700 text-sm mt-1">
                            Actualmente no es posible registrar o modificar tu disponibilidad.
                            Si necesitas realizar cambios urgentes, por favor <strong>contacte con el Área Académica</strong>.
                        </p>
                        {period.submissionStart && period.submissionEnd && (
                            <p className="text-xs text-red-600/80 mt-2 font-mono">
                                Ventana programada: {formatDateUTC(period.submissionStart)} - {formatDateUTC(period.submissionEnd)}
                            </p>
                        )}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Periodo Académico</p>
                            <p className="text-lg font-bold text-slate-800">{period.name}</p>
                            <p className="text-xs text-slate-400">{formatDateUTC(period.startDate)} - {formatDateUTC(period.endDate)}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${isOpenForSubmission ? 'bg-green-50 text-green-600' : 'bg-slate-100 text-slate-500'}`}>
                            {isOpenForSubmission ? <CheckCircle size={24} /> : <Lock size={24} />}
                        </div>
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Estado de Carga</p>
                            <p className={`text-sm font-bold ${isOpenForSubmission ? 'text-green-700' : 'text-slate-600'}`}>
                                {isOpenForSubmission ? 'Habilitada (Activa)' : 'Cerrada'}
                            </p>
                            {period.submissionEnd && (
                                <p className="text-xs text-slate-400">Cierre: {formatDateUTC(period.submissionEnd)}</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                    <div className="flex items-center gap-3">
                        <div className={`p-2 rounded-lg ${statusText.includes('Final') ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'}`}>
                            <BadgeCheck size={24} />
                        </div>
                        <div>
                            <p className="text-xs text-slate-500 uppercase font-bold">Tu Estado</p>
                            <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold ${statusColor} mt-1`}>
                                {statusText}
                            </span>
                        </div>
                    </div>
